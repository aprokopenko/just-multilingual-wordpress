...
function jcmst_map_posts($from, $to){
	...
	if( empty($errors) ){
		...
		if( empty($to_info['row']) ){
			...
			$wpdb->insert($wpdb->post_translations, 
				array(
					'blog_id' => $to['blog_id'],
					'post_id' => $to['post_id']
				),
		...
		if( !empty($to_info['row']) ) $rows_to_update = array_merge($rows_to_update, array($to_info['row']));
		if( !empty($to_info['chain']) ) $rows_to_update = array_merge($rows_to_update, $to_info['chain']);
		// add rows from new post and its chains
		if( !empty($from_info['row']) ) $rows_to_update = array_merge($rows_to_update, array($from_info['row']));
		if( !empty($from_info['chain']) ) $rows_to_update = array_merge($rows_to_update, $from_info['chain']);
